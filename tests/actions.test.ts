...
test("task.add writes a new list and leaves the old board alone", () => {
  ...
  const result = runAction(
    { do: "task.add", title: "Post the parcel", tag: "errand" },
    prev,
    NOW,
  )
  assert.equal(result.changed, true)
...
test("an ambiguous name changes nothing and says so", () => {
  ...
  assert.throws(
    () => runAction({ do: "task.done", title: "call" }, prev, NOW),
    /several tasks match/i,
  )
})
...
test("a reminder without a certain hour is refused, not guessed", () => {
  assert.throws(
    () =>
      runAction(
        { do: "reminder.add", title: "Dentist", when: "sometime next week" },
        frozen(),
        NOW,
      ),
...
test("a reminder in the past is refused", () => {
  assert.throws(
    () =>
      runAction(
        { do: "reminder.add", title: "Dentist", when: "9am yesterday" },
        frozen(),
        NOW,
      ),
    /already passed/i,
...
test("reminder.add saves a future due time and does not promise a nudge", () => {
  const result = runAction(
    { do: "reminder.add", title: "Dinner with Mum", when: "tomorrow at 6pm" },
    frozen(),
    NOW,
  )
  assert.equal(result.changed, true)
...
test("habit.done moves the streak once a day", () => {
  const prev = frozen({
    habits: [
      { id: "h", name: "Read", streak: 4, goal: 7, done: false, icon: "o" },
    ],
  })
...
test("spend.add stores money out as negative and moves the budget along", () => {
  const prev = frozen({
    budgets: [{ id: "b", category: "Dining", spent: 40, limit: 200 }],
  })
  const result = runAction(
    { do: "spend.add", label: "Ramen", amount: "$24.50", category: "dining" },
    prev,
    NOW,
  )
  assert.equal(result.changed, true)
  ...
  assert.equal(
    result.data.transactions[0]?.when,
...
test("budget.set creates one, then adjusts the same one", () => {
  const made = runAction(
    { do: "budget.set", category: "Coffee", limit: 60 },
    frozen(),
    NOW,
  )
...
test("journal, notes, brain and interests land on their own shelves", () => {
  const journal = runAction(
    { do: "journal.add", body: "A long day.", mood: "tired" },
    frozen(),
    NOW,
  )
  assert.equal(journal.data.journal[0]?.body, "A long day.")
  assert.equal(journal.data.journal[0]?.date, NOW.toLocaleDateString("en-AU"))
  const note = runAction(
    { do: "note.add", text: "Bin night is Tuesday" },
    frozen(),
    NOW,
  )
  assert.equal(note.data.voiceNotes[0]?.text, "Bin night is Tuesday")
  const gone = runAction(
    { do: "note.delete", text: "bin night is tuesday" },
    note.data,
    NOW,
  )
  assert.equal(gone.data.voiceNotes.length, 0)
  const brain = runAction(
    { do: "brain.note", text: "Sleeps badly on Sundays" },
    frozen(),
    NOW,
  )
  assert.equal(brain.data.brainNotes[0], "Sleeps badly on Sundays")
  const twice = runAction(
    { do: "brain.note", text: "sleeps badly on sundays" },
    brain.data,
    NOW,
  )
  assert.equal(twice.changed, false)
  const interest = runAction(
    { do: "interest.add", name: "Bouldering" },
    frozen(),
    NOW,
  )
...
test("spoken confirmations stay speakable and vary", () => {
  const prev = frozen({
    ...
    habits: [
      { id: "h", name: "Read", streak: 1, goal: 7, done: false, icon: "o" },
    ],
  ...
  const messages = [
    ...
    runAction(
      { do: "spend.add", label: "Ramen", amount: 24.5, category: "Dining" },
      prev,
      NOW,
    ).message,
...
test("parseActions leaves a plain reply alone", () => {
  const { spoken, actions } = parseActions(
    "Very good, sir. The kettle is your affair.",
  )
...
test("parseActions discards malformed JSON without breaking the reply", () => {
  const { spoken, actions } = parseActions(
    'Noted. [[ACT {"do":"task.add",,,}]] Anything else, sir?',
  )
...
test("parseActions reads a block whose text contains brackets", () => {
  const { spoken, actions } = parseActions(
    '[[ACT {"do":"note.add","text":"see ]] in the notes"}]] Down it goes.',
  )
...
test("a parsed action runs end to end", () => {
  const { spoken, actions } = parseActions(
    'Consider it done. [[ACT {"do":"task.add","title":"Ring the vet"}]]',
  )
...
const spoken = (error: unknown): string =>
...
test("by default he is still sir, in confirmation and in refusal alike", () => {
  const added = runAction(
    { do: "task.add", title: "Post the parcel" },
    frozen(),
    NOW,
  )
...
test("a chosen address is spoken in a confirmation and in a thrown refusal", () => {
  const prev = frozen({
    address: "Master",
    tasks: [{ id: "a", title: "Call mum", done: false }],
  })
  const added = runAction(
    { do: "task.add", title: "Post the parcel" },
    prev,
...
test("the regex fast path speaks the chosen address as well", () => {
  ...
  assert.throws(
    () => applyCommand("reminder", "remind me to ring the bank", prev, NOW),
    /Master/,
  )
  assert.throws(
    () => applyCommand("calendar", "book a table for two", prev, NOW),
    /Master/,
  )
  assert.throws(
    () =>
      applyCommand(
        "reminder",
        "remind me to ring the bank at 9am yesterday",
        prev,
        NOW,
      ),
    /Master/,
...
test("address.set accepts twenty-four characters and refuses twenty-five", () => {
  const prev = frozen({ address: "sir" })
  assert.equal(
    runAction({ do: "address.set", term: "M".repeat(24) }, prev, NOW).changed,
    true,
  )
  assert.throws(
    () => runAction({ do: "address.set", term: "M".repeat(25) }, prev, NOW),
    /twenty-four/i,
  )
})
