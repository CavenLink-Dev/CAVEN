...
export type CardKind = "reminder" | "tasks" | "habits" | "calendar" | "voicenote" | "finance" | "journal" | "brain"
...
const OFFLINE_LINE = (a: string) =>
  `I'm afraid I've lost the thread there, ${a}. Do give me a moment and try again.`
const NO_SPEECH_LINE = (a: string) =>
  `This browser won't let me listen, I'm afraid, ${a}. Do type to me instead.`
...
export function useCaven() {
  ...
  // glass panel) ~15x/second and made the UI crawl. Instead the live value lives
  // in a ref that the core reads in its own rAF loop, so audio frames never
  // re-render the board at all.
  ...
  const startMic = useCallback(() => {
    ...
    startListening(
      ...
      (fatal) => {
        ...
        resetAmp()
        setReply(
          `I can't get at your microphone, ${addressRef.current}. Grant me access, or simply type it.`,
        )
        set("idle")
      },
    )
  ...
  const rearm = useCallback(() => {
    ...
    setTimeout(() => {
      if (
        (conversingRef.current || lockedRef.current) &&
        ...
        try {
          ...
          line =
            error instanceof Error
              ? error.message
              : `That did not save, ${addressRef.current}. Please retry.`
        }
      }
