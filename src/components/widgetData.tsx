import { budgets, calendar, habits, tasks, transactions, voiceNotes } from '../lib/mockData';
...
export const WIDGETS: Widget[] = [
  ...
  { title: 'FINANCE', hint: 'Spending, bills, savings', body: (<div className="space-y-2">{budgets.slice(0, 2).map((b) => (<div key={b.id}><div className="mb-1 flex justify-between text-xs"><span>{b.category}</span><span className="opacity-70">${b.spent}/{b.limit}</span></div><Bar value={(b.spent / b.limit) * 100} /></div>))}<div className="text-xs opacity-70">Next bill: {transactions[3].label} ${Math.abs(transactions[3].amount)}</div></div>) },
  ...
  { title: 'VOICE NOTES', hint: 'Transcript + summary', body: (<div><div className="text-xs opacity-80">{voiceNotes[0].text}</div><div className="mt-1.5 text-[10px] tracking-widest" style={{ color: 'var(--caven-steel)' }}>{voiceNotes[0].when}</div></div>) },
