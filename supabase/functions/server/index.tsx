...
const EDWARD_VOICE =
  Deno.env.get("ELEVENLABS_VOICE_ID") ?? "xru6qZB94sJdkyqP12qN"
...
function mount(
  path: string,
  method: "get" | "post",
  handler: Parameters<typeof app.get>[1],
) {
...
async function elevenLabsSpeak(text: string): Promise<ArrayBuffer> {
  ...
  const res = await fetch(
    `https://api.elevenlabs.io/v1/text-to-speech/${EDWARD_VOICE}`,
    {
      method: "POST",
      headers: {
        "xi-api-key": apiKey,
...
mount("/state", "get", (c) =>
  c.json(
    {
      error: "retired",
      message: "Use Vercel /api/state with a signed-in account.",
    },
    410,
  ),
)
