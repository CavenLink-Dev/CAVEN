...
export function CavenStoreProvider({ children }: { children: ReactNode }) {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      if (!res.ok)
        throw new Error("Could not load your board. Retry before editing.")
      const body = await res.json()
      if (!mounted.current) return
      current.current = { ...seedData(), ...body.state }
      version.current = body.version
      setData(current.current)
      loaded.current = true
      setReady(true)
      setStatus("Saved to your private account")
    } catch (e) {
  ...
  const save = useCallback(async (next: CavenData) => {
    ...
    const res = await apiFetch("state", {
      method: "POST",
      body: JSON.stringify({ state: next, version: version.current }),
    })
    const body = await res.json()
    if (!res.ok) {
      if (res.status === 409) loaded.current = false
  ...
  const capture = useCallback(
    (kind: ActionKind, text: string) =>
      enqueue(async () => {
        try {
          const result = applyCommand(kind, text, current.current)
          if (result.changed) await save(result.data)
          return { message: result.message, changed: result.changed }
        } catch (e) {
      ...
      enqueue(async () => {
        try {
          ...
          for (const action of actions) {
            const result = runAction(action, next)
            next = result.data
            if (result.message) said.push(result.message)
            if (result.changed) changed = true
          }
          if (changed) await save(next)
          return { message: said.join(" "), changed }
        } catch (e) {
          setStatus(e instanceof Error ? e.message : "Save failed")
          throw e
        }
  ...
  return (
    <Context.Provider
      value={{ data, ready, status, reload, update, capture, perform }}
    >
      {children}
    </Context.Provider>
  )
...
export function useCavenStore() {
  ...
  if (!s) throw new Error("Missing CAVEN store")
  return s
}
