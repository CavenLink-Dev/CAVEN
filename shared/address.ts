...
export function addressOf(
  data: { address?: unknown } | null | undefined,
): string {
...
export type AddressCheck = { ok: true term: string } | {
  ok: false
  reason: string
}
...
export function checkAddress(raw: unknown, spokenTo: string): AddressCheck {
  ...
  if (/[\r\n\t\u0000-\u001f\u007f]/.test(source)) {
    return {
      ok: false,
      reason: `That is not a name I can use, ${spokenTo}. Nothing has changed.`,
    }
  }
  ...
  if (!term)
    return {
      ok: false,
      reason: `What should I call you, ${spokenTo}? Nothing has changed.`,
    }
  if (term.length > ADDRESS_LIMIT) {
    return {
      ok: false,
      reason: `Keep it to twenty-four characters or fewer, ${spokenTo}. Nothing has changed.`,
    }
  }
  ...
  if (!/^[\p{L}][\p{L} .'’-]*$/u.test(term)) {
    return {
      ok: false,
      reason: `Letters only for that, ${spokenTo}. Nothing has changed.`,
    }
  }
  ...
  if (BLOCKED.some((bad) => folded.includes(bad))) {
    return {
      ok: false,
      reason: `I would rather not, ${spokenTo}. Choose something I can say in company.`,
    }
  }
  return { ok: true, term }
}
