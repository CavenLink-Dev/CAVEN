...
const states: CoreState[] = ['idle', 'listening', 'thinking', 'speaking', 'success']
const CoreContext = createContext<{
  ...
  powerOff: () => {},
  lock: () => {},
  activeModules: { overview: true, tasks: true, calendar: true, reminders: true },
  toggleModule: () => {},
  setAllModules: () => {},
  lastAction: null,
})
...
export function CoreProvider({ children }: { children: ReactNode }) {
  ...
  const [activeModules, setActiveModules] = useState<Record<ActiveModule, boolean>>({
    overview: true,
    tasks: true,
    calendar: true,
    reminders: true,
  ...
  const [pulse, setPulse] = useState<{ id: number; type: PulseType }>({ id: 0, type: 'green' })
  ...
  const toggleModule = (mod: ActiveModule) => {
    setActiveModules(prev => {
      ...
      setLastAction(`CAVEN ${nextVal ? 'PROJECTED' : 'BEAMED OUT'} ${mod.toUpperCase()} BOARD`)
      return { ...prev, [mod]: nextVal }
    })
  }

  const setAllModules = (active: boolean) => {
    setActiveModules({ overview: active, tasks: active, calendar: active, reminders: active })
    setLastAction(active ? 'CAVEN RE-PROJECTED ALL HOLO-BOARDS' : 'CAVEN STOWED ALL HOLO-BOARDS')
  }
