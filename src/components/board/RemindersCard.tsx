  ...
  const ordered = useMemo(() => {
    return data.reminders
      .map((reminder, index) => {
        ...
        return {
          reminder,
          at: Number.isNaN(ms) ? Number.MAX_SAFE_INTEGER : ms,
          index,
        }
      })
    ...
    >
      <div className="space-y-3">
        {!ready ? (
          ...
          <PanelNote>
            No reminders armed. I'll hold one for you whenever you like.
          </PanelNote>
        ) : (
          ordered.map((reminder) => {
            const note = [reminder.date, reminder.note]
              .filter(Boolean)
              .join(" · ")
            return (
              <div className="flex gap-3" key={reminder.id}>
                ...
                <span>
                  <b className="block t-title text-white/90">
                    {reminder.title}
                  </b>
                  {note && (
                    <small className="t-caption text-white/45">{note}</small>
                  )}
                </span>
              </div>
            )
