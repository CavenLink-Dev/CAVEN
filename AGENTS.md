...
- Push needs `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` and `CRON_SECRET` in Vercel; `pnpm vapid` prints a key pair. Rotating the keys invalidates every browser subscription. `vercel.json` asks Vercel Cron for every minute, which a Hobby plan reduces to daily — run `supabase/scheduling.sql` once to schedule it from pg_cron instead. iOS grants web push only to a site added to the Home Screen; `needsHomeScreen()` says so rather than offering a switch that cannot work.
