...
const board = (address?: string): any => ({
  tasks: [],
  habits: [],
  reminders: [],
  calendar: [],
...
test("filters the inappropriate, including evasions", () => {
  ...
  for (const t of bad) {
    ...
    assert.ok(
      !r.ok && /nothing has changed|rather not/i.test(r.reason),
      `weak refusal for ${t}: ${!r.ok && r.reason}`,
    )
  }
})
...
test("rejects shape abuse that would poison the prompt", () => {
  for (const t of [
    ...
    assert.equal(
      checkAddress(t, "sir").ok,
      false,
      `let through: ${JSON.stringify(t)}`,
    )
})
...
test("nothing is hardcoded: the chosen term flows into speech", () => {
  ...
  assert.throws(
    () => runAction({ do: "task.done", title: "nope" }, m),
    /Master/,
...
test("address.set is filtered and sticks", () => {
  ...
  assert.throws(
    () => runAction({ do: "address.set", term: "fuckface" }, board()),
    /rather not/i,
  )
