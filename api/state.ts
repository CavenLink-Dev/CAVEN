...
export default async function handler(req: Request) {
  try {
    ...
    if (
      ...
      version < 0 ||
      !state ||
      fields.some((f) => !Array.isArray(state[f])) ||
      fields.some((f) => state[f].length > 2000)
    )
    ...
    if (
      state.reminders.some(
        (r: any) =>
          ...
          (r.dueAt && !Number.isFinite(Date.parse(r.dueAt))),
      )
    )
      return json({ error: "Invalid reminder" }, 400)
    const { data, error } = await db.rpc("save_caven_board", {
      ...
      return json(
        {
          error:
            error.code === "40001"
              ? "Your board changed on another device. Reload before retrying."
