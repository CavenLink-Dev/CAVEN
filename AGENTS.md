...
- `vercel.json` declares no cron. A Hobby plan reduces a job to daily, which is no use to a reminder due at 6:26pm, and a schedule that cannot do its job is worse than none. The pg_cron job in `supabase/migrations/20260912224518_schedule_caven_push_dispatch.sql` runs it every minute instead (live since 12 Sep). iOS grants web push only to a site added to the Home Screen; `needsHomeScreen()` says so rather than offering a switch that cannot work.
...
- `api/push-dispatch.ts` is the only **Node** function here; every other route is Edge. Two things differ and neither shows up in a build. It must export named `GET`/`POST` — a default export on Node is `(req, res) => void`, a returned `Response` is ignored, and the function then hangs until Vercel kills it at 300s (a 504 with nothing in the log about the real cause). And relative imports need explicit `.js`, per the rule above. Check a change to it by calling the deployed route, not by watching the build go green.
- Every relative import under `api/` and `shared/` ends in **`.js`** — not `.ts`, not bare. That is what is actually emitted, so it is right for both runtimes, and the two fail differently and late without it: `.ts` makes the Edge bundler refuse the **deploy** (`referencing unsupported modules`) after a green build, and a bare specifier makes Node throw `ERR_MODULE_NOT_FOUND` at **invocation** after a green deploy. `pnpm build` sees neither — it runs `tsc --noEmit && vite build` and never bundles a function. `npx vercel build --prod` does; run it before pushing anything that changes an `api/` import graph, and read `.vercel/output/functions/*/` to confirm each specifier points at a file that exists. (`shared/actions.ts` keeps `.ts` specifiers and is fine — no `api/` route imports it.)
