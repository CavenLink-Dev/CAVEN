...
export function applyCommand(
  ...
  const said = text.trim(),
    ...
    day = now.toLocaleDateString("en-AU"),
    unchanged = { data: prev, message: "Here it is.", changed: false }
  // Queries and negations never mutate data. Viewing and writing are separate operations.
  if (
    /^(show|what|how|when|where|do i|did i|have i|can you show|tell me|open|check|don'?t (add|save|create|log|delete)|do not)\b/i.test(
      said,
    )
  )
    return unchanged
  if (
    kind === "reminder" &&
    /\b(remind me|set a reminder|reminder for|nudge me|wake me|let me forget)\b/i.test(
      said,
    )
  ) {
    ...
    if (due.getTime() <= now.getTime())
      throw new Error(
        `That time has already passed, ${term}. Please give me a future date and time.`,
      )
    const title =
      (said.slice(0, match.index) + said.slice(match.index + match.text.length))
        .replace(
    ...
    return {
        ...
        reminders: [
          {
            ...
            time: due.toLocaleTimeString("en-AU", {
              hour: "numeric",
              minute: "2-digit",
            }),
      ...
      message: `It's on the board for ${due.toLocaleString("en-AU")}.`,
      changed: true,
    }
  }
  if (kind === "tasks") {
    const done =
      said.match(
        /^(?:please )?(?:tick|cross) (.+?) off(?: my (?:list|tasks))?$/i,
      ) || said.match(/^(?:complete|finish|mark complete) (.+)$/i)
    if (done) {
      const target = prev.tasks.filter(
        (t) => t.title.toLowerCase() === done[1].toLowerCase(),
      )
      if (target.length !== 1)
        throw new Error(
          `Please use the exact task name, ${term}. Nothing changed.`,
        )
      return {
        data: {
          ...prev,
          tasks: prev.tasks.map((t) =>
            t.id === target[0].id ? { ...t, done: true } : t,
          ),
        },
        message: "That's done.",
        changed: true,
    ...
    const add = said.match(
      /^(?:please )?(?:add (?:a |an )?task[: ]*|new task[: ]*|add )(.+?)(?: to my (?:tasks|list))?$/i,
    )
    if (add)
      return {
        data: {
          ...prev,
          tasks: [{ id, title: add[1], done: false }, ...prev.tasks],
        },
        message: "Noted. It's on the list.",
        changed: true,
      }
  }
  if (
    kind === "journal" &&
    /^(?:journal[: ]|(?:add|write|save|log)\b)/i.test(said)
  )
    return {
      data: {
        ...prev,
        journal: [
          {
            ...
            mood: "",
            title: "Journal entry",
            body: said.replace(/^journal[: ]*/i, ""),
          },
          ...prev.journal,
        ],
      },
      message: "It's in the journal.",
      changed: true,
    }
  if (
    kind === "voicenote" &&
    /^(?:make|take|save|add|note|jot|remember)\b/i.test(said)
  )
    return {
      data: {
        ...prev,
        voiceNotes: [{ id, text: said, when: day }, ...prev.voiceNotes],
      },
      message: "I've got that down.",
      changed: true,
    }
  if (kind === "calendar" && /^(book|schedule|add)\b/i.test(said))
    throw new Error(
      `Calendar booking is not connected yet, ${term}. I can save a reminder with a date and time.`,
    )
  return unchanged
}
...
function findOne<T>(
  ...
  const pool =
    exact.length > 0
      ? exact
      : rows.filter((row) => label(row).toLowerCase().includes(want))
  if (pool.length === 0)
    throw new Error(
      `I've no ${noun} by the name of ${needle}, ${term}. Nothing has changed.`,
    )
  if (pool.length > 1) {
    throw new Error(
  ...
  switch (verb) {
    case "task.add": {
      const title = required(
        action.title,
        `What should the task be, ${term}? Nothing has been added.`,
      )
      ...
      return {
        data: { ...prev, tasks: [task, ...prev.tasks] },
        message: `On the list, ${term}.`,
        changed: true,
      }
    }
    ...
    case "task.undone": {
      ...
      const title = required(
        action.title,
        `Which task did you mean, ${term}? Nothing has changed.`,
      )
      const target = findOne(prev.tasks, (t) => t.title, title, "task", term)
      if (target.done === wanted) {
        return idle(
          wanted
            ? `${target.title} was already crossed off, ${term}.`
            : `${target.title} is still open, ${term}.`,
        )
      }
      const tasks = prev.tasks.map((t) =>
        t.id === target.id ? { ...t, done: wanted } : t,
      )
      return {
        data: { ...prev, tasks },
        message: wanted ? "Crossed off." : "Back on the list.",
        changed: true,
      }
    }
    ...
    case "task.delete": {
      const title = required(
        action.title,
        `Which task should go, ${term}? Nothing has changed.`,
      )
      const target = findOne(prev.tasks, (t) => t.title, title, "task", term)
      return {
    ...
    case "reminder.add": {
      const title = required(
        action.title,
        `What should I remind you about, ${term}? Nothing has been saved.`,
      )
      const when = required(
        action.when,
        `What date and time should I remind you, ${term}? Nothing has been saved yet.`,
    ...
    case "reminder.delete": {
      const title = required(
        action.title,
        `Which reminder should go, ${term}? Nothing has changed.`,
      )
      const target = findOne(
        prev.reminders,
        (r) => r.title,
        title,
        "reminder",
      ...
      return {
        data: {
          ...prev,
          reminders: prev.reminders.filter((r) => r.id !== target.id),
        },
    ...
    case "event.add": {
      const title = required(
        action.title,
        `What shall I call it, ${term}? Nothing has gone in the diary.`,
      )
      const when = required(
        action.when,
        `What date and time is that, ${term}? Nothing has gone in the diary.`,
      ...
      const kind: CalendarEvent["kind"] =
        asked === "routine" || asked === "reminder" ? asked : "event"
    ...
    case "event.delete": {
      const title = required(
        action.title,
        `Which entry should go, ${term}? Nothing has changed.`,
      )
      const target = findOne(
        prev.calendar,
        (e) => e.title,
      ...
      return {
        data: {
          ...prev,
          calendar: prev.calendar.filter((e) => e.id !== target.id),
        },
    ...
    case "habit.add": {
      const name = required(
        action.name,
        `Which habit should I track, ${term}? Nothing has been added.`,
      )
      if (prev.habits.some((h) => sameText(h.name, name)))
        return idle(`I'm tracking ${name} already, ${term}.`)
    ...
    case "habit.done": {
      const name = required(
        action.name,
        `Which habit, ${term}? Nothing has changed.`,
      )
      const target = findOne(prev.habits, (h) => h.name, name, "habit", term)
      if (target.done)
        return idle(`${target.name} is already ticked today, ${term}.`)
      const streak = target.streak + 1
      const habits = prev.habits.map((h) =>
        h.id === target.id ? { ...h, done: true, streak } : h,
      )
      return {
        data: { ...prev, habits },
        message: `${target.name} ticked. That's ${streak} in a row.`,
        changed: true,
      }
    }
    ...
    case "habit.delete": {
      const name = required(
        action.name,
        `Which habit should go, ${term}? Nothing has changed.`,
      )
      const target = findOne(prev.habits, (h) => h.name, name, "habit", term)
      return {
        data: {
          ...prev,
          habits: prev.habits.filter((h) => h.id !== target.id),
        },
    ...
    case "journal.add": {
      const body = required(
        action.body,
        `What should I write, ${term}? Nothing has gone in the journal.`,
      )
      const entry = {
        id,
        date: day,
        mood: text(action.mood),
        title: text(action.title) || "Journal entry",
        body,
      }
      return {
    ...
    case "note.add": {
      const body = required(
        action.text,
        `What should the note say, ${term}? Nothing has been kept.`,
      )
      return {
        data: {
          ...prev,
          voiceNotes: [{ id, text: body, when: day }, ...prev.voiceNotes],
        },
    ...
    case "note.delete": {
      const body = required(
        action.text,
        `Which note should go, ${term}? Nothing has changed.`,
      )
      const target = findOne(prev.voiceNotes, (n) => n.text, body, "note", term)
      return {
        data: {
          ...prev,
          voiceNotes: prev.voiceNotes.filter((n) => n.id !== target.id),
        },
    ...
    case "spend.add": {
      const label = required(
        action.label,
        `What was it for, ${term}? Nothing has been logged.`,
      )
      const raw = amountOf(action.amount)
      if (raw === null || raw === 0)
        throw new Error(`How much was it, ${term}? Nothing has been logged.`)
      ...
      const budgets = budget
        ? prev.budgets.map((b) =>
            b.id === budget.id ? { ...b, spent: b.spent + spent } : b,
          )
        : prev.budgets
    ...
    case "budget.set": {
      const category = required(
        action.category,
        `Which budget, ${term}? Nothing has changed.`,
      )
      const limit = amountOf(action.limit)
      if (limit === null || limit <= 0)
        throw new Error(
          `What should the limit be, ${term}? Nothing has changed.`,
        )
      const existing = prev.budgets.find((b) => sameText(b.category, category))
      if (existing) {
        if (existing.limit === limit)
          return idle(
            `${existing.category} is already set at ${money(limit)}, ${term}.`,
          )
    ...
    case "brain.note": {
      const note = required(
        action.text,
        `What should I remember, ${term}? Nothing has been filed.`,
      )
      if (prev.brainNotes.some((n) => sameText(n, note)))
        return idle(`I have that one already, ${term}.`)
      return {
        data: { ...prev, brainNotes: [note, ...prev.brainNotes] },
        message: "Filed away.",
        changed: true,
      }
    }
    ...
    case "interest.add": {
      const name = required(
        action.name,
        `Which interest, ${term}? Nothing has changed.`,
      )
      if (prev.interests.some((i) => sameText(i, name)))
        return idle(`${name} is already down as an interest, ${term}.`)
      return {
        data: { ...prev, interests: [name, ...prev.interests] },
        message: `${name}, noted.`,
        changed: true,
      }
    }
    ...
    case "address.set": {
      ...
      return {
        data: { ...prev, address: verdict.term },
        message: `Very good, ${verdict.term}. I shall address you so from now on.`,
        changed: true,
      }
    }
...
export function parseActions(
  reply: string,
): { spoken: string actions: CavenAction[] } {
