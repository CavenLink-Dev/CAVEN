...
export function SettingsPage() {
  ...
  const saveAddress = async (event: React.FormEvent) => {
    ...
    if (!verdict.ok) {
      setAddressNote(verdict.reason)
      return
    }
    if (verdict.term === saved) {
      setAddressNote(`Already ${saved}.`)
      return
    }
    setAddressNote("")
      ...
      <section className="glass-panel holo-board holo-in">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-2 t-caption opacity-60">
            {addressNote ||
              `Sir, madam, Master \u2014 whatever suits. He can also be told out loud.`}
          </div>
        </div>
      </section>
          ...
          <div className="mt-3 flex items-center justify-between gap-4">
            <div>
              ...
              <div className="text-xs opacity-60">
                {muted ? "Muted." : "Audible, and kept quiet."}
              </div>
            </div>
            <button
              type="button"
        ...
        <div className="relative z-10">
          ...
          <div className="mt-3 flex items-center justify-between gap-4">
            <div className="min-w-0">
              ...
              <div className="text-xs opacity-60">
                Your board is private to this account.
              </div>
            </div>
            <button
              type="button"
      ...
      <section className="glass-panel holo-board holo-in">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-3 text-sm text-white/90">
            Edward, an English valet voice
          </div>
          <div className="mt-1 text-xs opacity-60">
            Speech is synthesised by ElevenLabs on the server. The voice and its
            credentials are configured there and are never sent to this browser.
            Should speech be unavailable, the browser's own voice stands in.
          </div>
        </div>
      </section>
    </div>
