...
function EyeIcon({ visible }: { visible: boolean }) {
  return visible ? (
    ...
    <svg
      ...
      className="size-5"
    >
      <path d="m3 3 18 18M10.6 6.2A9.7 9.7 0 0 1 12 6c6 0 9.5 6 9.5 6a16 16 0 0 1-3.2 3.8M6.2 6.8C3.7 8.5 2.5 12 2.5 12s3.5 6 9.5 6c.9 0 1.8-.2 2.6-.4" />
    </svg>
  )
}
...
export function AccountGate({ children }: { children: ReactNode }) {
    ...
    try {
      const result = signup
        ? await supabase.auth.signUp({
            ...
            options: {
              emailRedirectTo:
                import.meta.env.VITE_SUPABASE_REDIRECT_URL ?? location.origin,
            },
          })
  ...
  if (session)
    return (
      <div key={session.user.id} className="size-full">
        {children}
      </div>
    )
    ...
    <main className="auth-shell">
      ...
      <section className="auth-panel" aria-labelledby="auth-title">
        <div className="auth-header">
          <img
            className="auth-icon"
            src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-Ihy4CDV5qSbccIzEPhuVPOH6kdjCxy.png"
            alt=""
          />
          <img
            className="auth-wordmark"
            src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-lfCMMGnKCnOIDetSbTUMiC2ZSafAi1.png"
            alt="CAVEN"
          />
        </div>
        <div
        ...
        >
          <button
            ...
            className={`auth-tab${signup ? "" : " is-active"}`}
